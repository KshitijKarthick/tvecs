...
class LeipzigPreprocessor(BasePreprocessor):
    ...
    def __init__(
        ...
        if not os.path.exists(os.path.join(corpus_dir_path, preprocessed_corpus_fname)):
            ...
            self._leipzig_corpus_preprocess(corpus_fname, corpus_dir_path, encoding)
    ...
    def _leipzig_corpus_preprocess(
        self, corpus_fname, corpus_dir_path, encoding="utf-8"
    ):
        """
        ...
        with codecs.open(
            os.path.join(corpus_dir_path, corpus_fname), "r", encoding=encoding
        ) as file:
            line_split_list = file.read().split("\n")
            tab_split_list = [line.split("\t")[1] for line in line_split_list]
            extracted_corpus = "\n".join(tab_split_list)
            with codecs.open(
                os.path.join(corpus_dir_path, "%s.preprocessed" % corpus_fname),
                "w",
                encoding="utf-8",
