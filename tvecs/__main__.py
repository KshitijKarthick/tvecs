...
tvex_calls = {
    "preprocessor": {"func": preprocess_corpus, "result": None},
    "model_generator": {"func": model_generator, "result": None},
    "bilingual_generator": {"func": bilingual_generator, "result": None},
    "vector_space_mapper": {"func": map_vector_spaces, "result": None},
}
order_of_tvex_calls = [
    "preprocessor",
    "model_generator",
...
def args_parser():
    ...
    parser.add_argument(
        "-s", "--silent", help="silence all logging", action="store_true"
    )
    parser.add_argument(
        "-i", "--iter", help="number of Word2Vec iterations", default=5, action="store"
    )
    parser.add_argument(
        "-m1",
        "--model1",
    ...
    parser.add_argument(
        "-c", "--config", dest="config", help="config file path", action="store"
    )
    parser.add_argument(
        "-b",
        "--bilingual",
    ...
    if args.recommendations is True:
        logger.info(
            "Recommendation Engine: %s => %s" % (args.language1, args.language2)
        )
        while (
            int(
                raw_input(
            ...
            tvecs_vm.get_recommendations_from_word(word, pretty_print=True)
...
def evaluate(logger, args):
    ...
    for func_name in order_of_evaluation:
        ...
        if func_name is "preprocessor":
            ...
            tvex_calls[func_name]["result"] = (
                func(
                    preprocessed_corpus=tvex_calls["preprocessor"]["result"][0],
                    language=args.language1,
                    iterations=args.iter,
                ),
                func(
                    preprocessed_corpus=tvex_calls["preprocessor"]["result"][1],
